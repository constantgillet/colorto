...
1. Unless specified with `--no-verify`, automatically runs pre-commit checks:
   ...
   - `npm run test` to run test units
   - `npm run format` to make sure code is properly formatted
...
- `--no-verify`: Skip running the pre-commit checks (lint, build, generate)

## Important Notes

- By default, pre-commit checks (`npm run lint`, `npm run build`, `npm run format`) will run to ensure code quality
